...
if(isset($_POST["report"])){
	...
	if($status == "All"){
		...
		$sql = "select * from bank_transactions where type = '$status'";
	}
		
		$result = $conn->query($sql);
if($result->num_rows == 0){
			set_error_msg("<strong>Oops!</strong> No ".$status." Bank Transactions available!...");
			header("Location: Finance_Bank_Accounts_Report.php");
}
$pdf = new PDF('P','mm','Letter');
$pdf->AliasNbPages();
$pdf->SetTitle("Success Internation School - ".ucfirst($status)." Bank Transactions History");
$pdf->AddPage();
...
$pdf->Cell(100,15,ucfirst($status).' Bank Transactions',0,1);
$pdf->Cell(195 ,5,'',0,1);
$pdf->Cell(195 ,5,'',0,1);
...
$pdf->Cell(50, 5,'Bank Name',0,0);
$pdf->Cell(30, 5,'Branch Name',0,0);
...
while($row = $result->fetch_assoc()){
	$sql_p = "select * from bank_accounts where id ='".$row["acc_id"]."'";
	$result_p = $conn->query($sql_p);
	$row_p = $result_p->fetch_assoc();

	$pdf->SetFont('Arial','',9);
	$pdf->Cell(5, 5, $i, 0, 0);
	$pdf->Cell(35, 5, $row_p["account_no"], 0, 0);
	$pdf->Cell(50, 5, $row_p["bankname"], 0, 0);
	$pdf->Cell(30, 5, $row_p["branch"], 0, 0);
	$pdf->Cell(20, 5, number_format((float)$row["amount"], 2, '.', ''), 0, 0);
	$pdf->Cell(30, 5, $row["type"], 0, 0);
...
$pdf->SetFont('Arial','B',9);
	$pdf->Cell(5, 5,"", 0, 0);
	$pdf->Cell(35, 5,"Total Bank Transactions ", 0, 0);
	$pdf->Cell(50, 5, "", 0, 0);
	$pdf->Cell(30, 5, "", 0, 0);
