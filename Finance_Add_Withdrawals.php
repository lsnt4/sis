<?php 
	...
	$tot = 0;
	$income_tot = "select amount from bank_transactions where type='withdraw'";
	...
	if(isset($_POST["add"])){
		...
		$added_date = date("Y-m-d h:i:s",time());
			...
			$sql_insert = "insert into bank_transactions (acc_id,amount,type,added_by,added_date) values"
						  ." ('$account','$amount','withdraw','$added_by','$added_date')";
						  if($conn->query($sql_insert) == true){
							  		set_success_msg("<strong>Success!</strong> New Withdrawal has been successfully inserted!");
									header("Location: Finance_Add_Withdrawals.php");
							  }
						  else{
	...
	if(isset($_POST["delete"])){
		...
		$id = $_POST["dep_id"];
			$sql_insert = "delete from bank_transactions where id = '$id'";
						  if($conn->query($sql_insert) == true){
							  		set_success_msg("<strong>Success!</strong>  Withdrawal WIT".$id." has been successfully deleted!");
									header("Location: Finance_Add_Withdrawals.php");
							  }
						  else{
								  set_error_msg("<strong>Oops!</strong> Something went wrong!...!");
	...
	if(isset($_POST["pending"])){
		...
		$added_date = date("Y-m-d h:i:s",time());
			...
			$sql_insert = "Update bank_transactions set status='closed',verified_by='$added_by',verified_date='$added_date' where id = '$id'";
						  if($conn->query($sql_insert) == true){
							  		set_success_msg("<strong>Success!</strong>  Withdrawal WIT".$id." has been successfully Moved to Closed!");
									header("Location: Finance_Add_Withdrawals.php");
							  }
						  else{
								  set_error_msg("<strong>Oops!</strong> Something went wrong!...!");
	...
	if(isset($_POST["closed"])){
		...
		$added_date = date("Y-m-d h:i:s",time());
			...
			$sql_insert = "Update bank_transactions set status='pending',verified_by='$added_by',verified_date='$added_date' where id = '$id'";
						  if($conn->query($sql_insert) == true){
							  		set_success_msg("<strong>Success!</strong>  Withdrawal WIT".$id." has been successfully Moved to Pending!");
				...
				<div class="col-md-10">                    
                    <nav>
						<div class="nav nav-tabs" id="nav-tab" role="tablist">
                            ...
                            <a class="nav-item nav-link active"><strong> Withdrawals 
                            <?php 
							if($row_wit > 0){
								echo "<span class='badge badge-success badge-pill'> ".$row_wit." <span>";
							}
							?>
                            </strong></a>
					...
					<div class="tab-content">
                        ...
                        <form method="post" action="Finance_Add_Withdrawals.php">
								<div class="row">
									<div class="col-md-12">
										<div class="input-group mb-3">
											...
											<div class="input-group-append">
                                                    ...
                                                    <a class="btn btn-secondary" href="Finance_Add_Withdrawals.php"> Refresh </a>
											</div>
										</div>
										</div>
									</div>
							...
							<form method="post" name="AddIncome" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" onsubmit="return(incomeValidation());">
								...
								<div class="form-group row">
									...
									<div class="col-sm-10">
										<div class="form-row">
											<div class="col-md-6">
												<input id="incomeid" type="text" class="form-control" name="dep_id" value="<?php echo " WIT".$max; ?>"  readonly="readonly">
											</div>
										</div>
									</div>
								</div>
								...
								<div class="form-group row">
									<div class="col-sm-10">
                                    	<div class="form-row">
										<div class="col-md-3">
											<input id="income_submit" name="add" type="submit" class="btn btn-dark" value=" Add Withdrawal">
                                        </div>
                                        </div>
									</div>
                               	 </div>
                             ...
                             <?php
												if(isset($_POST["search"])){
													...
													if(empty($s_text)){
														...
														$sql_delete = "select * from bank_transactions where type='withdraw'";
													}else{
														$sql_delete = "select * from bank_transactions where ".$search." like '%".$s_text."%' and type='withdraw'";
													}
											...
											?>			<thead class="odd">
															<tr>
																<th scope="col"><center>Withdrawal ID</center></th>
											...
											?>
                                            			<tbody>
                                                            ...
                                                            <?php $inc_id = $row["id"]; ?>
																<th scope="row">WIT<?php echo $inc_id; ?></th>
                                                                ...
                                                                <th class="<?php echo $row["status"]; ?>"><?php echo $row["status"]; ?></th>
                                                   				<th scope="row">
                                             					<form action="Finance_Add_Withdrawals.php" method="post" onSubmit="return confirm('WARNING!\n\n1. Accidentally verifying or deleting of records cannot backup from the system.\n2. There is no way to undo this action.\n\nDo you still really want to continue WIT<?php echo $row["id"]; ?>?');">
