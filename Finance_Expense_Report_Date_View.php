...
if(isset($_POST["report"])){
	...
	if($startdate < $enddate){
		$sql = "select * from expenses where added_date between '$startdate' and '$enddate'";
		$result = $conn->query($sql);
if($result->num_rows == 0){
			set_error_msg("<strong>Oops!</strong> No Expenses available between '$startdate' and '$enddate'!...");
			header("Location: Finance_Expense_Report.php");
}
$pdf = new PDF('P','mm','Letter');
$pdf->AliasNbPages();
$pdf->SetTitle("Success Internation School - Expense History from $startdate to $enddate");
$pdf->AddPage();
...
$pdf->Cell(100,15,'Expense ('.$startdate.' - '.$enddate.')',0,1);
$pdf->Cell(195 ,5,'',0,1);
$pdf->Cell(195 ,5,'',0,1);
...
while($row = $result->fetch_assoc()){
	...
	$pdf->Cell(55, 5, $row["description"], 0, 0);
	$pdf->Cell(30, 5, $row["payment_method"], 0, 0);
	$pdf->Cell(15, 5, number_format((float)$row["amount"], 2, '.', ''), 0, 0);
	$pdf->Cell(20, 5, $row["paid_for"], 0, 0);
...
$pdf->SetFont('Arial','B',9);
	$pdf->Cell(5, 5,"", 0, 0);
	$pdf->Cell(35, 5,"Total Expenses ", 0, 0);
	$pdf->Cell(55, 5, "", 0, 0);
	$pdf->Cell(30, 5, "", 0, 0);
	$pdf->Cell(15, 5, number_format((float)$tot, 2, '.', ''), 0, 0);
	$pdf->Cell(20, 5, "", 0, 0);
	$pdf->Cell(30, 5, "", 0, 1);
$pdf->Close();
$pdf->Output();
